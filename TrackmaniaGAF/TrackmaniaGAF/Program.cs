...
namespace TrackmaniaGAF
{
    internal class Program
    {
        ...
        private static void Main(string[] args)
        {
            ...
            var population = new Population(100, 60, false, false);
        ...
        public static double EvaluateFitness(Chromosome chromosome)
        {
            ...
            if (chromosome != null)
            {
               ...
               //// int CheckPointGoal = vam.ReadInt32((IntPtr)cpg2 + 0x334);
                //Console.WriteLine(CheckPointGoal);

                //VAMemory vam = new VAMemory(process);
                //get x and y from the solution
                string binaryInstructions = chromosome.ToBinaryString(0, chromosome.Count);
                List<string> driveInstructions = new List<string>();
                for(int i = 0; i < binaryInstructions.Length; i+=3)
                {
                    driveInstructions.Add(binaryInstructions.Substring(i, 3));
                    //Console.Write(binaryInstructions.Substring(i, 4) + "-");
                 //   Console.Write(Convert.ToInt32(binaryInstructions.Substring(i, 4) + "-"), 2);
               //     Console.Write(Convert.ToInt32(binaryInstructions.Substring(i, 4) + "-"), 2));
                    Console.Write(Convert.ToInt32(binaryInstructions.Substring(i,3), 2) + " - ");
                    Thread.Sleep(500);
                }
                Console.WriteLine();
